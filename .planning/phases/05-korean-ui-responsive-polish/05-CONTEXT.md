...
### 모바일 레이아웃 전략
- **D-01:** 모바일(<768px)에서 응답 카드 2개는 **탭 전환** 방식으로 표시한다. [Model A] [Model B] 탭을 상단에 배치하고, 선택된 모델의 응답만 표시한다.
- **D-02:** 스트리밍 중에도 탭 전환이 가능하다. 두 응답이 모두 완료될 때까지 기다리지 않는다.
- **D-03:** 반응형 브레이크포인트는 **md (768px)** 기준이다. 768px 미만 = 모바일(탭), 768px 이상 = 데스크톱(가로 나란히). 태블릿 세로모드는 모바일로 취급한다.
...
- 모바일 탭 전환의 구체적 UI 구현 방식 (shadcn Tabs vs 커스텀)
